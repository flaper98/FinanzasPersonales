...
function TarjetaCard({
  ...
  const tieneUSD = tarjeta.saldoActualUSD > 0;
  const tcListo = !tieneUSD || (tipoCambio !== null && tipoCambio > 0);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
        <div>
          ...
          {tieneUSD && (
            <div className="text-xs text-slate-400">
              + {formatUSD(tarjeta.saldoActualUSD)}
              {tcListo && ` (≈ ${formatMonto(resumen.saldoActualUSDenPEN)})`}
            </div>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <div className="font-semibold text-slate-700">{tcListo ? formatMonto(resumen.deudaProyectada) : '—'}</div>
        </div>
        <div>
          <div className="text-xs text-slate-400">Disponible</div>
          <div className={`font-semibold ${tcListo && sobregirado ? 'text-rose-600' : 'text-emerald-600'}`}>
            {tcListo ? formatMonto(resumen.disponible) : '—'}
          </div>
        </div>
      </div>
      ...
      {!tcListo && (
        <p className="text-xs text-amber-700 bg-amber-50 rounded-lg px-3 py-2">
          Esperando el tipo de cambio (arriba) para calcular la deuda proyectada y el disponible combinando soles y
          dólares.
        </p>
      )}
